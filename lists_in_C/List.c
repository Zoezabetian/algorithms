...
List new(void) {
    // use L since the header file seems to like it. Shoul dwe use list instead?
...
int front(List L) {
    ...
    return L->front->index; //dereferences the list item at the front (index 0)
}
...
int back(List L) {
    ...
    return L->back->index; //dereferences the list item at the back of list
}
...
int get(List L) {
    ...
    return L->cursor->index; // dereferences the cursor position to access the index field of the node at that position 
}
...
void clear(List L) {
    ...
    while (length(L) > 0) { //ensures that all nodes in the list are removed
      deleteFront(L); 
    }
...
void set(List L, int x) {
    ...
    L->front->index = x; // set the value of the cursor element's index field (data) to the int value x
}
...
void toFront(List L) {
    if (L == NULL) {
      fprintf(stderr, "Error: calling toFront() on NULL List\n");
      return;
    }
    if (length(L) <= 0) {
      fprintf(stderr, "Error: calling toFront() on empty List\n");
      return;
    }
...
void toBack(List L) {
    if (L == NULL) {
      fprintf(stderr, "Error: calling toBack() on NULL List\n");
      return;
    }
    if (length(L) <= 0) {
      fprintf(stderr, "Error: calling toBack() on empty List\n");
      return;
    }
...
void previous(List L) {
    if (L == NULL) {
      fprintf(stderr, "Error: calling previous() on NULL List\n");
      return;
    }
    if (L->cursor == NULL) {
      return;
    }
    if (L->cursor == L->front) { // if cursor is at the front
      L->cursor = NULL; //previous would set cursor to null and index to -1 (undefined)
      L->index = -1;
    } else {
...
void next(List L) {
    if (L == NULL) {
      fprintf(stderr, "Error: calling next() on NULL List\n");
      return;
    }
    if (L->cursor == NULL) {
      return;
    }
    if (L->cursor == L->back) { // if cursor is at the back
      L->cursor = NULL; //next would set cursor to null and index to -1 (undefined)
      L->index = -1;
    } else {
...
void prepend(List L, int x) {
    ...
    n->index = x; //index of node (value stored) is x
    ...
    if (L->back == NULL) { // means the list was empty before n was inserted
...
void append(List L, int x) {
    ...
    if (L->back != NULL) { //if list is not empty
    ...
    if (L->front == NULL) { //means the list was empty before insertion
...
void insertBefore(List L, int x) {
    ...
    if (L->cursor->prev != NULL) { // if cursor is not at the front
...
void insertAfter(List L, int x) {
    ...
    if (L->cursor->next != NULL) { // if cursor is not at the back
...
void deleteFront(List L) {
    ...
    Node n = L->front;
    if (L->front == L->back) { // only one element, so lots of cleanup
      L->front = NULL;
      L->back = NULL;
      L->cursor = NULL;
      L->index = -1;
      ...
      L->front->prev = NULL;
      if (L->cursor != NULL) { // if cursor is defined, we need to update its index
        L->index--;
      }
    }
    L->length--;
...
void deleteBack(List L) {
    ...
    if (L->front == L->back) { // only one element in list
      ...
      if (L->cursor == n) { // if cursor is at the back
...
void delete(List L) {
    ...
    if (L->cursor == L->front) { //if cursor is at the front, delete the front item
      deleteFront(L);
...
void printList(FILE* out, List L) {
    ...
    while (n != NULL) { // as long as n is not NULL:
      fprintf(out, "%d ", n->index); //dereference pointer to access value stored in the index member of the node n is pointing to
      n = n->next; // move pointer so that n is now pointing to where n->next was pointing
    }
}
...
List copyList(List L) {
    ...
    List copy = new(); // make a new list called copy
    Node n = L->front; // set n to point to front of list
    while (n != NULL) { //as long as n is not null:  
      append(copy, n->index); // add index value pointed to by n to the back of list
