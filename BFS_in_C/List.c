...
//     }
// } changed  this to test list output for graph.c
void printList(FILE* out, List L) {
    if (L == NULL) {
        fprintf(stderr, "Error: calling printList() on NULL List\n");
        return;
    ...
    Node n = L->front; // make n the pointer pointing to front of list
    while (n != NULL) { // as long as n is not NULL:
        fprintf(out, "%d", n->index); // Print the value stored in the index member of the node
        if (n->next != NULL) { // Check if there is a next node
